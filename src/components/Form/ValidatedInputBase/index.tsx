...
const ValidatedInputBase = (props: ValidatedInputBaseProps) => {
  ...
  return <TextField {...props}>alkdflakjd</TextField>
}
...
//       const customValidationArray = Array.isArray(customValidation) ? customValidation : [customValidation]
...
//   const isNumber = type === 'number'
//   const onlyAcceptsNumbers = inputMode === 'numeric' || isNumber // some fields like phone numbers need numbers, but SF expects their value as a string
//   const ComponentName = onlyAcceptsNumbers ? NumberFormat : TextField

//   return (
//     <Field name={field} validate={validate}>
//       {({ field: { value, onBlur, name, onChange }, form: { touched, errors, setFieldValue, isSubmitting } }) => {
//         const adornmentPosition = icon ? `${icon.position}Adornment` : 'startAdornment'
...
//           [adornmentPosition]: (
//             <InputAdornment position={(icon && icon.position) || 'start'} className={isLarge ? classes.largeInput : ''}>
//               {isLoading ? (
//                 <CircularProgress size={20} />
//               ) : (
//                 <Icon color={icon.color} icon={icon.name} size={isLarge ? 'inherit' : undefined} />
//               )}
//             </InputAdornment>
//           )
//         }
...
//                 <Typography variant="body2" color="textSecondary">{`${counter} / ${maxLength} characters`}</Typography>
//               </div>
//             )}
//             <ComponentName
//               id={name}
//               {...{ required, select, multiline, name, placeholder, inputMode, value }}
//               // NumberFormat props
//               {...(isNumber
//                 ? {
...
//               helperText={(errors[field] && touched[field] && errors[field]) || helperText}
//               onBlur={onBlur(name)}
//               label={
//                 <span>
//                   {label}
